...
// 此文件会在 node_modules/html-minifier 中执行
...
var singleAttrIdentifier = /([^\s"'<>/=]+)/,
    ...
    qnameCapture = (function() {
      // 基于 https://www.npmjs.com/package/ncname
...
// 可自闭和标签
var closeSelf = makeMap('colgroup,dd,dt,li,option,p,td,tfoot,th,thead,tr,source,input');
...
// HTML5 标签 https://html.spec.whatwg.org/multipage/indices.html#elements-3
...
function HTMLParser(html, handler) {
  ...
  while (html) {
    last = html;
    // lastTap：上次解析的标签元素
    // special：检查上次解析的标签元素是否为特殊标签
    if (!lastTag || !special(lastTag)) {

      // 判断标签开始位置，如果不为0则表示当前字符串为一个元素的子内容
      ...
      if (textEnd === 0) {
        ...
        // 如果遇到了闭合标签，则截取该标签，并标识prevTap为闭合标签
        // parseEndTag：清理堆栈
        var endTagMatch = html.match(endTag);
        if (endTagMatch) {
          html = html.substring(endTagMatch[0].length);
          endTagMatch[0].replace(endTag, parseEndTag);
          prevTag = '/' + endTagMatch[1];
          continue;
        }

        // 如果遇到了开始标签，则截取该标签，并标识prevTap为开始标签
