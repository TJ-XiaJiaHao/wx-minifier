...
var selfClosingInlineTags = createMapFromString('comment,img,input,wbr,input');
...
function isAttributeRedundant(tag, attrName, attrValue, attrs) {
  attrValue = attrValue ? trimWhitespace(attrValue) : '';
...
function isScriptTypeAttribute(attrValue) {
  attrValue = trimWhitespace(attrValue.split(/;/, 2)[0]);
    ...
    new HTMLParser(input, {
      start: function(tag, attrs) {
        ...
        for (var i = 0, len = attrs.length; i < len; i++) {
          ...
          else if (options.processScripts && attr.name === 'type') {
            currentTag = tag;
